...
public class TableServiceTest {
    ...
    void 주문_테이블_생성() {
        ...
        // when
        OrderTable saveOrderTable = tableService.create(orderTable1);

        // then
        assertAll(
                () -> assertThat(saveOrderTable.getTableGroupId()).isNull(),
                () -> assertThat(saveOrderTable.getNumberOfGuests()).isEqualTo(orderTable1.getNumberOfGuests())
        );
    }
    ...
    void 주문_테이블_전체_목록_조회() {
        ...
        assertAll(
                ...
                () -> assertThat(findOrderTables).containsExactly(orderTable1, orderTable2)
        );
    }
    ...
    void 주문_테이블_상태_변경() {
        ...
        when(orderTableDao.save(orderTable1)).thenReturn(orderTable1);
        // when
        OrderTable resultOrderTable = tableService.changeEmpty(orderTable1.getId(), changeOrderTable);
        // then
    ...
    void 주문_테이블_방문한자수_변경(int expectNumberOfGuests) {
        ...
        when(orderTableDao.save(orderTable1)).thenReturn(orderTable1);
        // when
        OrderTable resultOrderTable = tableService.changeNumberOfGuests(orderTable1.getId(), changeOrderTable);
        // then
    ...
    void 주문_테이블_상태_변경_예외3() {
        ...
        // when & then
        Assertions.assertThatThrownBy(
                () -> tableService.changeEmpty(orderTable1.getId(), changeOrderTable)
        ).isInstanceOf(IllegalArgumentException.class);
    }
